...
import { buildCoverLetterDocx } from "@/lib/docx/coverLetterDocx";
...
/**
 * Generate a styled CoverLetter.docx on the fly from the folder's CoverLetter.md.
...
export async function GET(
  ...
  let buffer: Buffer;
  try {
    buffer = await buildCoverLetterDocx(coverLetter);
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
    console.error("[cover-letter.docx] generation failed:", detail);
    return NextResponse.json({ error: "Failed to render .docx", detail }, { status: 500 });
  }

  const download = req.nextUrl.searchParams.get("download") === "1";
  const fileName = safeFileName(`${app.company}_CoverLetter`) + ".docx";
